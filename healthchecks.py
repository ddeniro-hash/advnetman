...
class NetBoxIPFetcher:
    ...
    def fetch_ips(self):
        for device_name, info in self.devices.items():
            ip_address = self.get_ip(device_name, info["device_id"], info["interface_id"])
            if ip_address:
    ...
    def get_ip(self, device_name, device_id, interface_id):
        ...
        response = requests.get(self.api_url, headers=self.headers, params=params, verify=False)
        
        if response.status_code == 200:
            data = response.json()
            if data['results']:
                ip_address = data['results'][0]['address'].split('/')[0]  # Get the IP without suffix
                return ip_address
            else:
                print(f"{device_name}: No IP address found.")
                return None
        else:
            print(f"Failed to get IP for {device_name}. Status code: {response.status_code}")
            return None
...
class NetworkHealthCheck:
    ...
    def check_bgp_neighbors(self):
        ...
        if device_name == "r3" or "r4":
            ...
            print(f"BGP neighbor check skipped for {device_name}. Only R3 and R4 are supported.")
            return None
