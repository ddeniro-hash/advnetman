...
def health_check():
    ...
    for device in devices:
        try:
            ...
            device_eapi = pyeapi.connect(
                host=device["ip"], username=username, password=device["password"]
            )
...
def parse_ospf_neighbors(output):
    ...
    pattern = re.compile(
        r"^\s*(\d+\.\d+\.\d+\.\d+)\s+\d+\s+\S+\s+\d+\s+([\w\s/]+)\s+\d{2}:\d{2}:\d{2}",
        re.MULTILINE,
    )
...
def parse_bgp_neighbors(output):
    ...
    pattern = re.compile(
        r"^\s*(\d+\.\d+\.\d+\.\d+)\s+\d+\s+(\d+)\s+\d+\s+\d+\s+\d+\s+\d+\s+\S+\s+(\w+)",
        re.MULTILINE,
    )
